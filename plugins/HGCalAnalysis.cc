...
class HGCalAnalysis : public edm::one::EDAnalyzer<edm::one::WatchRuns, edm::one::SharedResources> {
 ...
 private:
  ...
  // helper classes
  //
  std::set<DetId> DetIdSignalRecHits;
...
void HGCalAnalysis::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  std::cout <<"  clear signal rec hits " << std::endl;
  DetIdSignalRecHits.clear();
  // 1st loop over calo particle; access signal rec hits
  if (readCaloParticles_) {
	...
	for (const auto& it_haf : hits_and_fractions) {
	  ...
	  DetIdSignalRecHits.insert(hitid);
	}
      }
    }
  }
  // 2nd standard loop over caloParticles
  if (readCaloParticles_) {
    for (std::vector<CaloParticle>::const_iterator it_caloPart = caloParticles->begin();
         it_caloPart != caloParticles->end(); ++it_caloPart) {
      const SimClusterRefVector simClusterRefVector = it_caloPart->simClusters();
  ...
  switch (algo_) {
	...
	//	std::cout <<"  EE det ID name = " << rechitsEE[i].detid().det() << std::endl; 
	if(DetIdSignalRecHits.find(rechitsEE[i].detid()) != DetIdSignalRecHits.end()) { 
	  hitmap_[rechitsEE[i].detid()] = &rechitsEE[i];
	}
      }
      for (unsigned int i = 0; i < rechitsFH.size(); ++i) {
	if(DetIdSignalRecHits.find(rechitsFH[i].detid()) != DetIdSignalRecHits.end()) { 
	  hitmap_[rechitsFH[i].detid()] = &rechitsFH[i];
	}
      }
      for (unsigned int i = 0; i < rechitsBH.size(); ++i) {
	if(DetIdSignalRecHits.find(rechitsBH[i].detid()) != DetIdSignalRecHits.end()) { 
...
void HGCalAnalysis::fillRecHit(const DetId &detid, const float &fraction, const unsigned int &layer,
  ...
  if(DetIdSignalRecHits.find(detid) == DetIdSignalRecHits.end()) {return;}
